...
// open the database
  ...
  Future<List<Model>> getAllFav() async {
    ...
    if (result.isNotEmpty) {
      modelList = result.map((item) {return Model.fromDB(item as Map<String, dynamic>);}).toList();
    }
    return modelList;
  }

  Future<int> setFav(String? station_id, String? name, String? desc, String? image, String? radio_url) async {
    var dbClient = await (db);
